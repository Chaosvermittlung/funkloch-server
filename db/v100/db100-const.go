...
--
-- File generated with SQLiteStudio v3.0.7 on Do. Jan. 5 18:40:01 2017
--
...
-- Table: Fault
CREATE TABLE Fault (faultid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, status INTEGER NOT NULL DEFAULT (0), comment TEXT);
...
-- Table: Faultlist
CREATE TABLE Faultlist (faulitd INTEGER REFERENCES Fault (faultid) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL, storeitemid INTEGER REFERENCES Storeitem (storeitemid) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL, PRIMARY KEY (faulitd, storeitemid));
...
-- Table: Participant
CREATE TABLE Participant (userid INTEGER NOT NULL REFERENCES User (userid) ON DELETE CASCADE ON UPDATE CASCADE, eventid INTEGER NOT NULL REFERENCES Event (eventid) ON DELETE CASCADE ON UPDATE CASCADE, arrival DATETIME NOT NULL, depature DATETIME NOT NULL, PRIMARY KEY (userid, eventid));
...
-- Table: Storeitem
CREATE TABLE Storeitem (storeitemid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, equipmentid INTEGER REFERENCES Equipment (equipmentid) ON DELETE CASCADE ON UPDATE CASCADE, storeid INTEGER REFERENCES Store (storeid) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL);
